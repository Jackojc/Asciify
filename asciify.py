...
class asciify():
    ...
    def text(self,x,y,string,center=(0,0),padchars=("",""),transbg=True,color=None,vertical=False,shadow=1,shadowcolor=(20,20,20)):
        ...
        if center[1]:
            if vertical:y = x + round(((self.CellsHeight/2) - (len(string)/2)))
            else:y = y + round((self.CellsHeight/2))
        else:y = y
        if center[0]:
            if vertical:x = y + round((self.CellsWidth/2))
            else:x = x + round(((self.CellsWidth/2) - (len(string)/2)))
        else:x = x
