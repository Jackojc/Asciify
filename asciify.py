...
class asciify():
    ...
    def __init__(
        self, width, height, font, margin=0, fontsize=16, bgcolor=(
            0, 0, 0), color=(
            255, 255, 255), offsets=(
                0, 0, 0, 0), aa=True):
        ...
        self.Foreground = pygame.Surface(
            self.Screen.get_size(), pygame.SRCALPHA, 32)
    ...
    def setCell(self, x, y, string=None, color=None):
        ...
        if string is None:
            pygame.draw.rect(self.Background, color,
                             (posX, posY, self.CellX, self.CellY))
        else:
            string = string[0]
            string = self.Font.render(string, self.Alias, color)
            self.Foreground.blit(
                string, (posX + self.OffsetX, posY + self.OffsetY))
    ...
    def setStringBlock(self, x, y, string=None, color=None, bgcolor=None):
        ...
        pygame.draw.rect(self.Background, bgcolor,
                         (posX, posY, self.CellX, self.CellY))
        string = string[0]
        string = self.Font.render(string, self.Alias, color)
        self.Foreground.blit(
    ...
    def setBlock(self, x, y, color=None):
        ...
        pygame.draw.rect(self.Foreground, color,
                         (posX, posY, self.CellX, self.CellY))
    ...
    def textFixed(
        ...
        if shadow:
            ...
            self.Foreground.blit(
                text, (posX + self.OffsetX, posY + self.OffsetY + shadowdist))

        text = self.Font.render(string, self.Alias, color)
        self.Foreground.blit(text, (posX + self.OffsetX, posY + self.OffsetY))

    def text(
        self, x, y, string, center=(
            0, 0), padchars=(
            "", ""), transbg=True, color=None, vertical=False, shadow=1, shadowcolor=(
                20, 20, 20)):
        if color is None:
            color = self.DefColor
        ...
        for num, char in enumerate(string):
            if transbg:
                if not vertical:
                    if shadow:
                        self.setStringBlock(
                            x + num, y + 0.07, char, color=shadowcolor)
                ...
                if not vertical:
                    if shadow:
                        self.setStringBlock(
                            ...
                            bgcolor=self.DefBgColor)
                    self.setStringBlock(
                        x + num, y, char, color=color, bgcolor=self.DefBgColor)
                else:
                    if shadow:
                        self.setStringBlock(
                            x, y + num + 0.07, char, color=shadowcolor)
    ...
    def box(self, x, y, w, h, filled=1, color=None, bg=0):
        ...
        if filled:
            ...
            if bg:
                pygame.draw.rect(self.Background, color,
                                 (posX, posY, posW, self.CellY))
                pygame.draw.rect(self.Background, color,
                                 (posX, posY, self.CellX, posH))
                pygame.draw.rect(
                    self.Background,
                    color,
                    (posX,
                     ...
                     self.CellY))
                pygame.draw.rect(self.Background, color,
                                 (posW + posX, posY, self.CellX, posH))
            else:
                pygame.draw.rect(self.Foreground, color,
                                 (posX, posY, posW, self.CellY))
                pygame.draw.rect(self.Foreground, color,
